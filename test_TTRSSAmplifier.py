...
def ampl_feeder_gen(item):
    return ExceptionFeederFilter(
        RSSHubFeeder(item['link'], logging.getLogger("test_RSSHubFeeder Amplifier: %s" % item['link'])),
        rate=0.2
    )
...
ttrss_amp_feeders = [
    AmplifierFeeder(
        ExceptionFeederFilter(
            TTRSSCatFeeder(
                cat_id=ttrss['cat_id'],
                logger=logging.getLogger("test_TTRSSFeeder - Amplifier %s %d" % (ttrss['url'], ttrss['cat_id'])),
                url=ttrss['url'],
